...
pub async fn handle(args: InstallArgs, vault_dir_override: Option<&str>) -> Result<()> {
    ...
    let name = args.name.clone().unwrap_or_else(|| get_default_name(&source));
...
fn parse_source(source: &str, source_type_override: Option<&SourceType>) -> Result<McpSource> {
    if let Some(st) = source_type_override {
        return match st {
            SourceType::Npm => Ok(McpSource::Npm { package: source.to_string() }),
            SourceType::Pypi => Ok(McpSource::PyPi { package: source.to_string() }),
            SourceType::Local => Ok(McpSource::Local { path: PathBuf::from(source) }),
            SourceType::Github => {
            ...
            SourceType::Docker => Ok(McpSource::Docker { image: source.to_string() }),
        };
    }

    if let Some((prefix, rest)) = source.split_once(':') {
        match prefix.to_lowercase().as_str() {
            "npm" => return Ok(McpSource::Npm { package: rest.to_string() }),
            "pypi" => return Ok(McpSource::PyPi { package: rest.to_string() }),
            "local" => return Ok(McpSource::Local { path: PathBuf::from(rest) }),
            "github" => {
            ...
            "docker" => return Ok(McpSource::Docker { image: rest.to_string() }),
            _ => {}
        }
    }

    if source.starts_with('/') || source.starts_with("./") || source.starts_with("../") || (source.contains('/') && !source.contains(':') && std::path::Path::new(source).exists()) {
        Ok(McpSource::Local { path: PathBuf::from(source) })
        ...
        Ok(McpSource::Npm { package: source.to_string() })
    }
}
...
fn get_default_name(source: &McpSource) -> String {
    match source {
        ...
        McpSource::Docker { image } => {
            image.split(':').next().and_then(|i| i.split('/').next_back()).unwrap_or(image).to_string()
        }
    }
}
