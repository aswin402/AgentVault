...
pub trait AgentConnector: Send + Sync {
    ...
    async fn write_config(&self, config: &AgentConfig) -> Result<(), VaultError> {
        ...
        let temp_path = config_path.with_extension("vault-tmp");
    ...
    async fn sync(&self, entries: &[McpEntry]) -> Result<SyncResult, VaultError> {
        ...
        let valid = self.verify().await?;
        if !valid {
            if let Some(ref bp) = backup_path {
                tokio::fs::copy(bp, config_path)
                    .await
                    .map_err(VaultError::Io)?;
            }
    ...
    async fn verify(&self) -> Result<bool, VaultError> {
        let config_path = self.config_path();
        if !config_path.exists() {
            return Ok(false);
        }
        ...
        match serde_json::from_str::<serde_json::Value>(&content) {
            Ok(v) => {
                ...
                if let Some(mcp_servers) = v.get("mcpServers") {
                    ...
                    if serde_json::from_value::<HashMap<String, serde_json::Value>>(
                        mcp_servers.clone(),
                    )
                    .is_err()
                    {
